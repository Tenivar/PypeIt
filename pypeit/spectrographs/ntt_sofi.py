...
class NTTEFOSC2Spectrograph(spectrograph.Spectrograph):
    """
    Child of Spectrograph to handle NTT/SOFI specific code
    """
    ndet = 1  # Because each detector is written to a separate FITS file
    telescope = telescopes.NTTTelescopePar()
    name = 'ntt_sofi'
    header_name = 'SOFI'
    camera = 'SOFI' # I don't know if this is correct, the camera used is called HAWAII ? 
    url = 'https://www.eso.org/sci/facilities/lasilla/instruments/sofi.html'
    supported = False
    comment = 'The ESO Son of ISAAC (SOFI) spectrograph'
